...
exports.createDocument = async (req, res) => {
  try {
    ...
    // if (!allowedTypes.includes(Description)) {
    //   return res.status(400).json({ message: 'Invalid Description value.' });
    // }

    const sql = `INSERT INTO Trash (DocID, Description, CreatedAt, FileName, FilePath) VALUES (?, ?, ?, ?, ?)`;
...
exports.getAllDocuments = async (req, res) => {
  try {
    const [rows] = await pool.execute(`SELECT * FROM Trash ORDER BY created_at DESC`);
    res.json(rows);
  } catch (error) {
...
exports.getDocumentById = async (req, res) => {
  try {
    const { id } = req.params;
    const [rows] = await pool.execute(`SELECT * FROM Trash WHERE id = ?`, [id]);
    if (rows.length === 0) {
...
exports.updateDocument = async (req, res) => {
  try {
    ...
    // const allowedTypes = ['EBM', 'Report', 'Delivery Note'];
    // if (Description && !allowedTypes.includes(Description)) {
    //   return res.status(400).json({ message: 'Invalid Description value.' });
    // }

    const sql = `UPDATE Trash SET DocID = ?, Description = ?, CreatedAt = ? WHERE id = ?`;
    const [result] = await pool.execute(sql, [
      DocID,
      Description,
      CreatedAt,
...
exports.deleteDocument = async (req, res) => {
  try {
    ...
    const [result] = await pool.execute(`DELETE FROM Trash WHERE id = ?`, [id]);
    if (result.affectedRows === 0) {
      return res.status(404).json({ message: 'Document not found' });
    }
